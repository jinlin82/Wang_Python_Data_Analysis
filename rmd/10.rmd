...
###  股票行情数据分析
####  历史行情数据分析
get_hist_data()可获取个股历史交易数据，可以通过参数设置获取日k线、周k线、月k线、5分钟、30分钟等数据，不过只能获取近3年的日线数据，如果需要全部历史数据，可以使用接口get_h_data()。

参数说明如下:
- code：股票代码，即6位数字代码，或者指数代码（如hs300=沪深300指数）；
- start：开始日期，格式YYYY-MM-DD；
- end：结束日期，格式YYYY-MM-DD；
- ktype：数据类型，D=日k线，W=周，M=月，5=5分钟，30=30分钟，默认为D；
...
####  实时行情数据分析
一次性获取当前交易所有股票的行情数据（如果是节假日，则为上一
交易日，结果显示速度取决于网速）
```{python,eval=F}
'''
